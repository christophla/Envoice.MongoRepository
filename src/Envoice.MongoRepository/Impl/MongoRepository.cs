...
namespace Envoice.MongoRepository.Impl
{
      ...
      where T : IEntity<TKey>
  {
    ...
    {
      return this.collection.FindSync<T>(GetIDFilter(id), options, cancellationToken).Single();
    }
    ...
    {
      return await this.collection.Find<T>(GetIDFilter(id), options).SingleAsync<T>(cancellationToken);
    }
